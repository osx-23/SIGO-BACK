...
public class GestionarAsistenciaService
        implements GestionarAsistenciaUseCase {

    private final AsistenciaGestionPort gestionPort;
    ...
    @Override
    @Transactional
    public ConsultarAsistenciasUseCase.Asistencia actualizar(
            Long id,
            Command command
    ) {
        Command validado = validar(command);
        Long actualizado = gestionPort.actualizar(id, validado);
        return consultaUseCase.obtenerPorId(actualizado);
    }
    ...
    private Command validar(Command command) {
        ...
        for (AusenciaCommand ausencia : ausencias) {
            if (ausencia == null
