...
public interface InventarioProductoRepository
        extends JpaRepository<InventarioProducto, Long> {
    ...
    @Query("""
        ...
        where p.activo = true
          and pr.rol.id = :rolId
          and pp.plaza.id = :plazaId
          and (
              upper(pr.rol.codigo) <> 'AGENTE'
              or (
                  p.ambito is not null
                  and upper(p.ambito.nombre) = 'RECURRENTES'
              )
          )
        order by p.nombre
    """)
    List<InventarioProducto> buscarPermitidos(
    ...
    @Query("""
        ...
        order by p.nombre
    """)
    List<InventarioProducto> listarAdministracionPorPlaza(
            @Param("plazaId") Long plazaId
    );
}
